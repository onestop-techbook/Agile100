...
なのですが、「良いらしい」とかねがね噂に聞いていたKintone SIGNPOST@<fn>{signpost}書籍版(Ver1.1)を
...
Kintone(平たく言えばWebアプリケーション開発基盤）における「継続的な業務改善」ですって？　どうにもアジャイルの
雰囲気がしてきますね。ここからは次から具体的なKintone SIGNPOSTの内容について軽く攫ってみましょう。
...
//footnote[パッケージベスプラ][パッケージソフトウェアでは２割以上改造するならフルスクラッチで全部作った方が工数的に有利になると一般的に言われていますね。]
...
//footnote[開発環境ランドスケープ][個人的にはSAP R/3やS/4HANAみたいに３システムランドスケープを置こう！みたいなレベル感で、複数ステージ環境強烈に取れる方が良い面もあるというのは非常に理解できるところではあります。まあ、３環境も取るのはやり過ぎでは……という話もわかる所ではあるので、現実的にはテスト/本番の２環境ぐらいに落ち着くのかなと思いますが。最近はいい時代になりましたよね……]
