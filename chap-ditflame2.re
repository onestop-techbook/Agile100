...
実は私、この本の執筆過程@<fn>{history}で転職する事になりました。次職ではKintoneを触る事になりそうなのですが、「良いらしい」とかねがね噂に聞いていた
Kintone SIGNPOST@<fn>{signpost}書籍版(Ver1.1)をこの度入手し、読んでみたところなるほどな……となるポイントが色々とありました。


この本を手に取られる方はしっかりしたアジャイル開発をされてる方も多いのだろうと思うのですが、そういった方にも相当に本書の内容は有用だろうと考えましたので、ここで紹介したいと思います。

//footnote[history][もともと、本書は2022年2月に開催予定の技書博６で発行予定でしたがコロナ禍で開催が中止となり、その後技術同人誌系のイベントがしばらく開催されなかった経緯もあって執筆時期が長くなり……という経緯があります。]

//footnote[signpost][@<href>{https://kintone.cybozu.co.jp/kintone-signpost/, Kintone SIGNPOST https://kintone.cybozu.co.jp/kintone-signpost/} なおWeb版は無料で誰でも読めます。輪読するとかであれば書籍の方が良いかもしれないです。]
...
Kintone(平たく言うとWebアプリケーション開発基盤）における「継続的な業務改善」ですって？　どうにもアジャイルの雰囲気がしてきますね。ここからは次から具体的なKintone SIGNPOSTの内容について軽く攫ってみましょう。
...
//image[cycle][全体でのサイクル][scale=0.8]


ステップは大きく７つ(kintone概念理解、目的設定、プロジェクト企画、設計と構築、リリースと定着、運営、継続企画)の分野に分かれています。それぞれのステップについて（私が気になったところを中心に）紐解いてみましょう。
...
「2-16 オープンな閲覧権限」@<fn>{オープンな閲覧権限}は情報共有の一貫ですし、「2-17 未来の変化への備え」@<fn>{未来の変化への備え}はもうそのまんまアジャイルですね。
「2-20 小さなリリース単位」@<fn>{小さなリリース単位}もXPでいうところの@<strong>{小さなリリース}そのものですね。


また、Kintoneならではのトピックがあるのも面白いところではあります。（「2-14 基本機能から考える」@<fn>{基本機能から考える}などは
パッケージソフトウェア利用時におけるベストプラクティスそのものです。パッケージソフトウェアでは２割以上改造するならフルスクラッチの方が工数的に有利になると一般則では言われていますね。）
...
特徴的だなと感じたものでいうと、例えば「3-26 担当別アプリ」@<fn>{担当別アプリ}でしょうか。アプリケーション設計においては共通部分は得てして共通化したい！
となるケースが多いですが@<fn>{プログラミング作法}、Kintoneは平易に使えるように設計・実装している事もあり、
むしろ@<strong>{担当が違うなら分離してそれぞれの取り回しを良くしましょう}というのが大方針として提示されている点については、Kintoneならではという感じがあります。


また、アジャイルと直接密な関係は無いような気がしますが、たとえば３案以上の案を考えてトレードオフを検討しよう　という「3-22 ３つ以上のアイデア」@<fn>{３つ以上のアイデア}や
平易に開発を行うための「3-32 開発環境の用意」@<fn>{開発環境の用意}は大事ですね。（個人的にはSAP R/3やS/4HANAみたいに３システムランドスケープを置こう！みたいなところまで
激しく複数ステージ環境を取れる方が良い面もあるというのは非常に理解できるところではあります。まあ、３環境も取るのはやり過ぎでは……という話もわかる所ではあるのですけど。）
...
パターン数は少ないのですが、筆者としては個人的に秀逸な視点が多いと感じたセクションです。「4-33 現場代表」@<fn>{現場代表}などはまさにアジャイルプロセスで速攻使うべきレベルですね。
「利用者サイドの人に使い方を説明してもらいましょう」というのが要旨なのですが、個人的にはあまり各種アジャイルプロセスではそこまで明確でなかった論点のような気がしています。
どうなんでしょうか。（まあ、利用者サイドも巻き込んでXP的に開発を進めた場合。利用者側が説明する事に自然となる様な気はするのですけれど。）


また「4-35 要望箱アプリ」@<fn>{要望箱アプリ}ではユーザーニーズの継続的な取得、また要望や不安を集め、定着率向上のために情報収集アプローチをしましょう　という話が説かれていたり、
「4-36 利用率の把握」@<fn>{利用率の把握}で利用率を把握し、それを元に改善方針・優先順位を決定しましょうという話は大事ですね。
...
「5-39 トラブル対応フロー」@<fn>{トラブル対応フロー}は一般的なシステムでも必要な内容ですね。普通の情報システムであれば運用管理の観点から設計・実装・運用がなされているかと思いますが、
Kintoneを使う層にはそういった観点が薄いだろうという考えが背景にあるのだろうなという事が想定されます。こういった基礎的な内容も丁寧に説明が入っている点は好感が持てます。
...
「6-40 担い手を増やす」@<fn>{担い手を増やす}では利用者をどんどん開発者にしよう　という話が説かれます。なんともXPっぽい感じがります。
恐らくですが、どんどんペアプログラミングや相互レビューで開発者が増えていくというのがモデルケースとして想定されているんですね。
