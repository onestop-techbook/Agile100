...
「あります。いい本ですよね、あれ（他人事）」と返答したところ、アジャイルプロジェクトに放り込まれた。（脚注1・2）
チームメンバー全員、WFしかやったことがなかったため、『アジャサム』の内容に従ってプロジェクトをやりながら、
「このプロジェクトってアジャイルになってるの？」「アジャイラー（脚注3）ってホントにこの世に存在してるの？」という
...
ハンドルネームを適当に作り、アカウントを作成、アジャイルビギナーOKのアジャイルイベント（脚注4）を申し込んだ。
...
進行の方は本日のイベントのアジェンダをモニターに映して説明、イベントは本日の発表（脚注6）になる。
...
この回答者の方は「スクラムマスター（？）」（脚注7）だと名乗られ、とても口調がやさしく、穏やかに話される。
