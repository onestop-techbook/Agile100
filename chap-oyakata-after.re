...
普通のソフトウエア開発において（チームや会社によって流儀はあるにせよ）、Masterへのマージは、本番環境へのリリースに近い意味があるでしょうし、開発ブランチで様々な検証が行われることでしょう。

一方で本においては、表に見えない/検証が必要な目に見えないバグというものはほぼ存在しません。であれば、Master一本で運用して人の目に触れるようにした方が確実にブラッシュアップされます。コメントも付けやすいですよね。他の人のネタ・原稿からアイディア、インスピレーションを得て自分の原稿に反映したり、といったプロセスも期待しています。

あとは、編集側としても、直接修正、あるいは最新バージョンを見て修正のお願いをするなどの運用が楽になります。

また、本書では、細かい編集や体裁上の調整、誤字修正は編集長の方で行いますというのを明言して進めました。

どこをどう直したのかがわかりやすいというメリットもありますが、それ以上に編集などの面倒なところをやらなくて済むようにすることで、ネタ出し、執筆に集中してほしい、出せるものを出し切ってほしい、という意図があります。通常、合同誌の原稿は「完成原稿を集めます」という運用が普通です。そのレベルまでもっていくのが大変、という心理的ハードルもあるでしょう。困ったことがあっても相談しづらい、指定された形式で書く必要があって学習コストが高いなどハードルになります。これに対し、編集・組版は編集担当でやりますので、Markdown、プレーンテキスト、GoogleDocなどなんでもOKという運用で底の部分のハードルも下げるように意図しています。執筆と編集は全く別のワークです。
