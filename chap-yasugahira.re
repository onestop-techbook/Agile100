...
メンバのスキルを星取表の形で可視化するスキルマップ@<fn>{skilmap}。チーム発足時のメンバのスキルの可視化、離任によるリスクの可視化、学習モチベーションの向上などを目的に作成、運用しているチームは多いのではないでしょうか？

//footnote[skilmap][スキルマップ自体の説明は、ryuzeeさんのブログエントリ スキルマップ作成のすすめ をご覧いただくのが良いと思います。https://www.ryuzee.com/contents/blog/7065]

スキルマップの運用について考えていた際、実はスキルマップにチームのアジリティ@<fn>{agility}が表れていて、それを認識した上でスキルマップを運用することで、チームのアジリティを向上させることができるのでは？という仮説が浮かんだので、この記事で説明します。


//image[skilmap][チーム発足時のスキルマップ例][scale=0.95]

//footnote[agility][この記事において「チームのアジリティ」とは、メンバが何かを提案してから、チームとして合意して具体的なアクションに繋げるまでの期間を指しています。この定義は私の勝手なイメージなで出典はないので、一般的なイメージと異なっていたらすみません。。。なおチームのアジリティという考え方は、広木さんの「2つのDX」とDX Criteriaのお話を聞いていて思いついた考え方です。]
...
//image[roll][他のロールのスキルを有したスキルマップ例][scale=0.95]
