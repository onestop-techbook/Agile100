...
どんな善意がどう裏目に出てしまったのか。アプローチの良し悪しはチームや状況でも変わってきますが、その一例として考察を挙げていきます。  
...
透明性(Transparency)、検査(Inspection)、適応(Adaptation)の3つの柱。スクラム初学者が「なかなかイメージしにくい」という話しをし、徐々に理解していこうとします。しかし、理解できるかに関わらず、これを意識して活動出来るか否かは大きな差を産むのではないでしょうか。
...
言葉に踊らされないようにと、スクラム特有の言葉を使わずに進んだ結果、この重要な相互協力のための形が失われました。イメージがしにくいなりに、そしてこれはスプリントゴールやプロダクトゴールに向かえているかと同様に繰り返し考え直したい内容です。スクラムが実現したい姿、そしてなぜ自分達がスクラムをやっているかをチーム考えることで、よりチームをよりよくするプロセスが働くのでしょう。  
...
前述の通り、毎朝の会も、毎週の会も、ただの報告会となっています。この状況下では、チーフからのアドバイスは叱責に、困りごとの共有はチーフへのお伺いのような形になります。この状態が始まると、チーフが善意を持ってアドバイスを試みても、メンバーは萎縮し、さらに自律性を失っていくことになります。結果、メンバーにとってはどんどんとスクラムが辛いものになり、チーフにとってもその状況を見るのが辛くなっていきます。  
...
「週次ミーティングでの成果に対する追求」を極度に恐れるようになっていたチームは、その後しばらく、週次ミーティングはタスクの完了と、次の週のやることの整理のみを行うようになりました。しかし次第に「今週終わらなかったから来週引き続き」ということが多くなってきました。

そこでチームは進め方を見直し、「スプリントレビュー」に定義される成果物の確認を計画に入れました。これは同時に、プラニングで「完了の定義」を行うことにつながります。チームは、これを通して毎週何が出来るかを意識し、個人で悩みすぎずにチームでどうやって達成するかを考えやすくなりました。
