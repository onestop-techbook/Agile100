...
「このプロジェクトってアジャイルになってるの？」「アジャイラー@<fn>{2}ってホントにこの世に存在してるの？」という
素朴かつ原始的な疑問がアジャイルビギナー100％のチームにあふれかえった。

//footnote[1][この話は化石エンジニア・アンモナイトの個人的な体験です。また、現職前の話です。WF開発を否定する意図はまったくありません。アンモナイトが過去の記憶を表現するとこのようになった、というだけです。]

//footnote[2][アジャイラー＝「アジャイル開発実践者」の仮名称。]
...
進行の方は本日のイベントのアジェンダをモニターに映して説明、イベントは本日の発表@<fn>{5}になる。

//footnote[4][慎重なら勢いだけでイベントに行かない。]

//footnote[5][当時「LT（Lightning Talk）」という単語を知らないため、「発表」と表現しました。]
...
「これをやればどんなチームでも間違いなくアジャイルになる、という方法はありますか？」@<fn>{6}
...
「エレベーターピッチの『重要な利点』と『差別化の決定的な特徴』の違いは何ですか？」等々。
...
//footnote[6][『アジャイルサムライ』の最後の衝撃（アジャイルかどうかの判断を読者の自問にゆだねる）に囚われて、思考が先に進まなくなっていました。]

//footnote[7][『アジャイルサムライ』には「スクラム」と「スクラムマスター」の名前の記載はありますが、「スクラムマスターは～（略）～ような感じだ。」で情報が終わるため、よくわかっていませんでした。]
...
コミュニティにも入り、社内の複数プロジェクトでスクラムマスターをやりました。
...
長らくWFの沼にたゆたっていたが、ある日アジャイルと出会い、自分が化石エンジニア＝生きるレガシーであることを知った。 気づかぬうちにIT系としてのレベルがLv3（推定）くらいになっている。このままではヤバイ、レガシーはいずれ駆逐される。 生き残りを目指して、唐突に化石→人類を目指して進化を始めました。社内プロジェクトでスクラムマスターを経験後、認定スクラムマスター（CSM）取得。Quitaでアジャイル本の地図を作ったり、技術力Upのため、Kaggle挑戦や他社研修の視聴等をやっています。
