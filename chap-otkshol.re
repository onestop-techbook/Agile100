...
はじめまして。@otkshol(おたけ)と申します。開発メンバーの一員として日々奮闘しております。本章では、朝会などの定期的に開催されている開発イベントの目的を答えることができなかったことをきっかけにアジャイル開発と初めて向き合った話を共有します。
...
昨年、私は別プロダクトのバッチ開発担当から新しい開発チームにアサインされました。5名のチームに対して、自分を含めた3名の新規着任、サブリーダー相当の方が1名離任するという規模の大きいメンバー入れ替えが発生したタイミングでの着任になります。
...
事前に出社日を調整して開発チームキックオフを対面開催しました。午前中はキックオフを行い、午後から「我々はなぜここにいるのか」を開発チーム で実施しました。議論は白熱して、半日以上の時間をかけてメンバー全員で合意した回答とまとめることができました。
...
しかし、今の僕たちには立ち返るチーム目標があ流ので、議論が発散しそうになったり、判断に迷った時は立ち返り着実に議論を進めることができるようになりました。これは大きな一歩なのではないかと思います。
