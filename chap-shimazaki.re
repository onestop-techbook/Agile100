...
当時のスクラムガイド @<fn>{scrumguide2017} 上、スクラムチームの構成は、下記のように表現されていました。
...
自己組織化とはなんなのか？どのような状態なのか？Wikipedia@<fn>{wiki_selfmanagement}上では下記のように記載されています。
...
しばらく自己組織化について調べると、「ティール組織」@<fn>{teal}に出会いました。特にこの中でも、ティール組織になるためのブレイクスルーが重要でした。
